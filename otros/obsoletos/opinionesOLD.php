					    ...
					    <div id="zonaFecha">
							 <h3>Escoger fecha de trabajo</h3>
								<select id="listaFechas">
									...
									<option id="last" value="09-03-2015">La última</option>
									</optgroup>
  ...
  <script>     
     ...
     $(document).ready(function() {  		 
		 ...
		 $("#listaFechas")
			.selectmenu({
				...
				select: function( event, ui ) {
					...
					eligeFechaTrabajo(); // función que elige la fecha de trabajo según la selección actual...
				    $("#borrartodo").click(); // borra todos los items
			    	$("#editor").editable("setHTML", "", true); // campo texto lo pone a cero
		...
		$("#contenedorOpinionesElegidas").droppable({  
		 ...
		 // Elección de una fecha de trabajo...
	 ...
	 function insertarOpinion(idAlumno,cadenaItems) {
		     // alert($("#editor").editable("getHTML", false, false)); // testeo cómo se envía el HTML del campo de escritura
	 ...
	 function obtenerListaFechas(idAlumno) {
		 ...
		 var posting = $.post( "./opiniones/scripts/obtenerFechas.php", { 
		  ...
		  posting.done(function(data,status) { 
			  // alert(data);
			  var datos = jQuery.parseJSON(data);
			  $("#listaFechas").find("#last").val(datos.last); // dato de la última fecha en formato DatePicker
			  ...
			  $(".fechasListado").each(function(){ // por cada elemento de esta clase...
				  if($.inArray($(this).attr("id"),datos.fecha)>=0) {
					  ...
					  // alert("Sí "+$(this).attr("id"));					  
				  } else {
					  $(this).attr('disabled',true);
					  // alert("No "+$(this).attr("id"));	
				  }	  
			  }); 
			  $("#listaFechas").selectmenu( "refresh" ); // ¡¡¡Dios dos días hasta conseguir ésto !!
		  });  
